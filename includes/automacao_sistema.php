<?php
/**
 * APPO - Automação do Sistema
 * Ficheiro: includes/automacao_sistema.php
 */
...
function processarLembretesAutomaticos($pdo) {
    ...
    $sql = "SELECT id_marcacao, id_utilizador, data, hora 
            FROM marcacoes 
            WHERE estado = 'Confirmada' 
    ...
    foreach ($marcacoes as $m) {
        $mensagem = "Lembrete: Tem uma marcação agendada para amanhã às " . date('H:i', strtotime($m['hora'])) . ".";
